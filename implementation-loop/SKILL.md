...
4. Record the effective model and reasoning effort for the parent, `implementer`, and `reviewer` at loop start when the harness exposes them. If a value is unavailable, record `unknown`; never infer historical settings from a later configuration snapshot.
5. If the Plan is absent, ambiguous, still under discussion, materially incomplete, or not testable as written, stop before spawning an agent. State what is missing and ask only for the decision needed to proceed.
6. Do not create, expand, reinterpret, or approve the Plan inside this skill. A necessary material deviation requires user approval and ends the current loop.
...
3. Require the implementer to translate every acceptance criterion into observable checks, including relevant success paths, boundary cases, failure injection, security/privacy behavior, cleanup, and non-mutating compile or static checks. Record any behavior that cannot be automated as a manual acceptance check instead of simulating success.
4. Require the implementer to run the existing suite and the new tests separately. Existing tests must still pass. Classify each new result as `EXPECTED_FAIL`, `UNEXPECTED_FAIL`, or `PASS`, and explain why each expected failure demonstrates missing production behavior rather than a broken test.
...
6. Spawn one `reviewer` with the Plan, tests-only diff, existing and new test results, manual acceptance checks, repository instructions, and pre-existing changes. The reviewer must inspect the tests independently and perform a complete review rather than stopping after the first finding.
...
1. Send the approved Plan, locked test baseline, manual acceptance checks, repository boundaries, and test-review result to the same `implementer`. Ask it to implement only the Plan, make the approved tests pass, add any non-weakening regression tests, and run all permitted validation.
2. Require an implementer self-check before review: compare the full diff with the Plan, confirm the locked tests are unchanged, exercise each defined failure injection, run safe compile/static checks even when live E2E is prohibited, and distinguish tested behavior from unverified live behavior.
...
5. Spawn a fresh `reviewer` with the packet. Count this as one implementation-review cycle. Require a complete, evidence-backed review of the full diff, tests, failure paths, documentation, and scope boundaries.
6. Parse the reviewer's exact `status`:
   ...
   - Missing, malformed, or contradictory status: ask the same reviewer once to restate the result. If it remains invalid, stop and report the protocol failure.
7. After a third `CHANGES_REQUIRED`, stop without further edits as `REVIEW_LIMIT_REACHED`. Escalate the remaining findings and test state to the user.
...
- final state: `PASS`, `BLOCKED`, `TEST_DESIGN_BLOCKED`, or `REVIEW_LIMIT_REACHED`;
